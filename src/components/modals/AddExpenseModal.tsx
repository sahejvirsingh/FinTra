...
const AddExpenseModal = ({
  ...
  const currencySymbol =
    SupportedCurrencies[profile.currency as keyof typeof SupportedCurrencies]
      .symbol;
  ...
  useEffect(() => {
    if (isEditing) {
      ...
      setItems(
        expenseToEdit.expense_items?.map(({ name, price, quantity }) => ({
          name,
          price,
          quantity,
        })) || []
      ...
      const isStandardCategory = ExpenseCategories.includes(
      ...
      if (defaultAccountId) {
        ...
        const checking = accounts.find(
          (acc) => acc.type === "Checking" || acc.name === "Checking Account"
        );
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? `Failed to save expense: ${err.message}`
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (extractedDetails) {
          ...
          if (extractedDetails.amount)
            setAmount(String(extractedDetails.amount));
          if (extractedDetails.category) {
            const matchedCategory = ExpenseCategories.find(
              (c) =>
                c.toLowerCase() === extractedDetails.category?.toLowerCase()
            );
            if (matchedCategory) {
          ...
          if (extractedDetails.items)
            setItems(
              extractedDetails.items.map((item) => ({
                name: item.name,
                price: item.price,
  ...
  const itemInputClasses =
    "bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none";
  ...
  const {
    ...
    const origPercent =
      income > 0 ? Math.max(0, Math.min(100, (bal / income) * 100)) : 0;
    const newPercent =
      income > 0 ? Math.max(0, Math.min(100, (newBal / income) * 100)) : 0;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        >
              ...
              {receiptPreview && (
                <div className="mb-4 flex justify-center">
                  <Image
                    ...
                    height={160}
                    width={200}
                    // className="max-h-40 w-auto mx-auto rounded-lg shadow-md object-contain"
                    priority
                  />
                </div>
              )}
              {!receiptProcessed && (
                <>
                  ...
                  <button
                    ...
                    disabled={anyLoading}
                    className="w-full flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isAiProcessing ? (
                      <>
                      ...
                      <>
                        ...
                        <span className="font-semibold text-sm">
                          Upload receipt to auto-fill details
                        </span>
                      </>
                    )}
                  </button>
          ...
          <div>
            <label htmlFor="account" className={labelClasses}>
              Account *
            </label>
            <CustomDropdown
          ...
          {selectedAccount && monthlyIncome > 0 && (
            <div className="space-y-2">
              <label className={labelClasses}>
                Balance Preview (vs. Monthly Income)
              </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {category === "Other" && (
              <div className="md:col-span-2">
                <label htmlFor="customCategory" className={labelClasses}>
                  Custom Category *
                </label>
                <input
                  type="text"
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="flex items-center text-sm font-semibold text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300"
              >
            ...
            <div className="space-y-3">
              {items.map((item, index) => (
                <div
                  key={index}
                  className="grid grid-cols-[1fr,90px,90px,auto] gap-3 items-center"
                >
                  <input
                    type="text"
                    value={item.name}
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`${itemInputClasses} pl-7`}
                      placeholder="Price"
                      step="0.01"
                    />
                  </div>
          ...
          {error && (
            <p className="text-sm text-red-400 text-center">hello{error}</p>
          )}
        </form>
