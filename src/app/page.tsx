...
export default function Home() {
  ...
  const fetchData = useCallback(
    async (isRefresh: boolean = false) => {
      if (!profile || !currentWorkspace.id || !supabase) return;
      ...
      try {
        const accountsPromise = supabase.rpc("get_accounts_for_workspace", {
          p_workspace_id: currentWorkspace.id,
        });
        const goalsPromise = supabase
          .from("goals")
          .select("*, goal_payments(*)")
        ...
        const topupsPromise = supabase
          ...
          .select("*")
          .eq("workspace_id", currentWorkspace.id);
        const recurringIncomesPromise = supabase
          .from("recurring_incomes")
          .select("*")
          .eq("workspace_id", currentWorkspace.id);
        const predictedBudgetsPromise = supabase.rpc("get_predicted_spending", {
          p_workspace_id: currentWorkspace.id,
        });
        ...
        if (accountsResult.error)
          throw new Error(`Accounts: ${accountsResult.error.message}`);
        if (goalsResult.error)
          throw new Error(`Goals: ${goalsResult.error.message}`);
        if (expensesResult.error)
          throw new Error(`Expenses: ${expensesResult.error.message}`);
        if (topupsResult.error)
          throw new Error(`Topups: ${topupsResult.error.message}`);
        if (recurringIncomesResult.error)
          throw new Error(
            `Recurring Incomes: ${recurringIncomesResult.error.message}`
          );
        if (predictedBudgetsResult.error)
          throw new Error(
            `Predicted Budgets: ${predictedBudgetsResult.error.message}`
          );
        ...
        const freshData = {
          ...
          recurringIncomes:
            (recurringIncomesResult.data as unknown as RecurringIncome[]) || [],
          predictedBudgets:
            (predictedBudgetsResult.data as unknown as PredictedBudget[]) || [],
        };
    ...
    [profile, currentWorkspace.id, supabase]
  );
  ...
  const addAccount = useCallback(
    async (
      ...
      if (!profile || !currentWorkspace.id)
    ...
    [profile, fetchData, currentWorkspace.id, supabase]
  );
  ...
  const handleViewItemHistory = useCallback(
    (item: ExpenseItem) => {
      const itemToView = aggregatedItems.find(
        (agg) => agg.name.toLowerCase() === item.name.toLowerCase()
      );
      if (itemToView) {
  ...
  const addExpense = useCallback(
    async (newExpenseData: NewExpenseData) => {
      if (!profile || !currentWorkspace.id)
        throw new Error("User or workspace not found");
    ...
    [fetchData, profile, supabase, currentWorkspace.id]
  );
  ...
  const addTopUp = useCallback(
    ...
    [closeTopUpModal, fetchData, profile, currentWorkspace.id, supabase]
  );
    ...
    >
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* add this into currency attribute: profile.currency */}
          <StatCard
            title="Total Income"
            value={totalIncome}
            currency={"USD"}
          ...
          <StatCard
            ...
            value={netFlow}
            currency={"USD"}
            colorClass={netFlow >= 0 ? "text-blue-500" : "text-orange-500"}
            period="this month"
          />
        </div>
        ...
        <BudgetOverview
          ...
          currency={"USD"}
          onNavigate={() => router.push("/budgeting")}
        />
        <FinancialGoals
          goals={goals}
          currency={"USD"}
          onNavigate={() => router.push("/financial_goals")}
        />
        <SpendingAnalytics expenses={expenses} currency={"USD"} />
      </div>
      {isExpenseModalOpen && profile && (
        <AddExpenseModal
          accounts={accounts}
