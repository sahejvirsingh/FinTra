...
const AnalyticsPage = ({ refreshTrigger }: { refreshTrigger: number }) => {
    ...
    const fetchData = useCallback(async (isRefresh: boolean = false) => {
        ...
        const cacheKey = `fintra_analytics_data_${currentWorkspace.id}`;
        ...
        try {
            ...
            const expensesPromise = supabase.from('expenses').select('*').eq('workspace_id', currentWorkspace.id).gte('date', yearAgo.toISOString().split('T')[0]);
            const topupsPromise = supabase.from('topups').select('*').eq('workspace_id', currentWorkspace.id).gte('topup_time', yearAgo.toISOString().split('T')[0]);
            ...
            const freshData = {
                ...
                netWorthHistory: (netWorthData as unknown as NetWorthDataPoint[]) || [],
            };
    ...
    const { totalExpense, totalIncome, categoryData, incomeVsExpenseData } = useMemo(() => {
        ...
        return { totalExpense: totalExp, totalIncome: totalInc, categoryData: catData, incomeVsExpenseData: incVsExpData };
    }, [filteredData]);
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-wrap justify-between items-center gap-4">
                ...
                <div className="flex items-center flex-wrap gap-2 bg-gray-200 dark:bg-gray-800 p-1 rounded-full">
                    {(['month', '3-month', 'year', 'custom'] as TimeFilter[]).map(filter => (
                        <button key={filter} onClick={() => setTimeFilter(filter)} className={`px-4 py-1.5 text-sm rounded-full transition-colors font-semibold capitalize ${timeFilter === filter ? 'bg-white dark:bg-gray-900 shadow' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-300/50 dark:hover:bg-gray-700/50'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard title="Total Income" value={totalIncome} currency={profile.currency} colorClass="text-green-500" />
                <StatCard title="Total Expenses" value={totalExpense} currency={profile.currency} colorClass="text-red-500" />
                <StatCard title="Net Flow" value={totalIncome - totalExpense} currency={profile.currency} colorClass={totalIncome - totalExpense >= 0 ? 'text-blue-500' : 'text-orange-500'} />
            </div>

             <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
                <h2 className="text-xl font-semibold mb-4 flex items-center gap-3 text-gray-900 dark:text-white"><TrendingUp className="text-indigo-500"/>Net Worth Over Time</h2>
                <ResponsiveContainer width="100%" height={350}>
                    <AreaChart data={filteredNetWorth} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                        <defs>
                        ...
                        <XAxis dataKey="snapshot_date" stroke={tickColor} fontSize={12} tickLine={false} axisLine={false} tickFormatter={(dateStr) => new Date(dateStr).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}/>
                        <YAxis stroke={tickColor} fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => formatCurrency(Number(value), profile.currency)} />
                        <Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#1f2937' : '#ffffff', border: '1px solid #374151', borderRadius: '0.75rem', padding: '10px' }} formatter={(value: number) => [formatCurrency(value, profile.currency), 'Net Worth']} labelStyle={{fontWeight: 'bold'}}/>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                 <div className="lg:col-span-3 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
                    <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Income vs. Expense</h2>
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={incomeVsExpenseData} margin={{top: 5, right: 20, left: 0, bottom: 5}}>
                            ...
                            <YAxis stroke={tickColor} fontSize={12} tickLine={false} axisLine={false} tickFormatter={val => formatCurrency(val, profile.currency)}/>
                 ...
                 <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
                    <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Spending by Category</h2>
             ...
             {currentWorkspace.type === 'organization' && (
                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
                    <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Project Budget Overview</h2>
