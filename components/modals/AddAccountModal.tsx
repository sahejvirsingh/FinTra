...
import { Account, AccountTypes, SupportedCurrencies, iconMap, accountTypeToIconName } from '../../types';
import CustomDropdown from '../shared/CustomDropdown';
import { useUser } from '../../contexts/UserContext';

type NewAccountData = Omit<Account, 'id' | 'created_at' | 'user_id' | 'workspace_id'>;
...
const AddAccountModal = ({ onClose, onAddAccount }: AddAccountModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? `Failed to add account: ${err.message}` : 'An unknown error occurred.');
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6" id="add-account-form">
          <div>
            <label htmlFor="name" className={labelClasses}>Account Name *</label>
            <input type="text" id="name" value={name} onChange={e => setName(e.target.value)} className={inputClasses} placeholder="e.g., Chase Checking" required />
          </div>
          
          <div>
            <label htmlFor="type" className={labelClasses}>Account Type *</label>
            <CustomDropdown 
          ...
          {type === 'Other' && (
            <div>
              <label htmlFor="customType" className={labelClasses}>Custom Account Type *</label>
              <input type="text" id="customType" value={customType} onChange={e => setCustomType(e.target.value)} className={inputClasses} placeholder="e.g., Vacation Fund" required />
            </div>
          )}

          <div>
            <label htmlFor="balance" className={labelClasses}>Initial Balance *</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-500 dark:text-gray-400">{currencySymbol}</span>
              <input type="number" id="balance" value={balance} onChange={e => setBalance(e.target.value)} className={`${inputClasses} pl-8`} placeholder="0.00" required step="0.01" />
            </div>
          </div>
        ...
        <div className="flex items-center justify-end p-6 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-b-2xl">
          ...
          <button type="submit" form="add-account-form" disabled={isSubmitting} className="px-6 py-2.5 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 transition-colors ml-4 w-36 h-[42px] flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
            {isSubmitting ? <Loader2 size={20} className="animate-spin" /> : 'Add Account'}
          </button>
        </div>
      </div>
    </div>
